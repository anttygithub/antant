...
type AsZoneObjectOption struct {
	...
	ObjId      int       `orm:"column(obj_id)" description:"对象id"`
	OptName    string    `orm:"column(opt_name);size(200)" description:"操作名"`
	Protocol   string    `orm:"column(protocol);size(200)" description:"协议"`
	Ip         string    `orm:"column(ip);size(50)" description:"服务地址"`
	Port       string    `orm:"column(port);size(10)" description:"服务端口"`
	Uri        string    `orm:"column(uri);size(200)" description:"URI"`
	Method     string    `orm:"column(method);size(50)" description:"方法（POST,GET,PUT,DELETE......）"`
	Comment    string    `orm:"column(comment);size(500);null" description:"说明"`
	IsActive   int8      `orm:"column(is_active)" description:"是否启用,1启用0不启用,默认不启用"`
	AddUser    string    `orm:"column(add_user);size(128);null"`
	UpdateUser string    `orm:"column(update_user);size(128);null"`
	CreateTm   time.Time `orm:"column(create_tm);type(datetime);auto_now_add"`
	UpdateTm   time.Time `orm:"column(update_tm);type(datetime);auto_now"`
}
